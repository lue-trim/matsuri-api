...
def request_api(record_end_time:datetime.datetime, room_id, jsonl_path):
    '构建请求并向服务端发送'
    # 构建请求
    data_id = str(uuid.uuid4())
    data_date = record_end_time.strftime(r"%Y-%m-%d %H:%M:%S.%f+08:00")
    data = {
        "id": data_id,
        "date": data_date,
...
def main():
    ...
    for xml_path in parse_list:
        ...
        request_api( record_end_time=end_time, jsonl_path=xml_path[:-3]+"jsonl", room_id=room_id)
